...
type listInputs={
    list:items[];
    setList:reactSetState<items[]>;
}

export const ItemList = ({list,setList}:listInputs) => {
    const handleDelete=(id:string)=>{
        setList((prev)=>prev.filter(data=>data.id!==id))
    }
  return (
    list.map((data)=>(
        <div className="flex flex-row justify-center items-center border mb-2 border-slate-600 px-6 rounded">
        <p key={data.id} className="text-6 mr-4 w-full" >{data.title}</p>
        <Button className="w-12 h-12 object-contain" onClick={()=>handleDelete(data.id)}>
          <svg xmlns="http://www.w3.org/2000/svg" x="1px" y="1px" width="30" height="30" viewBox="0 0 100 100">
